# D
# 训练时将本文件RENDER=False，Switch=0，以及player.py中的dt=1
# 测试时将本文件RENDER=True，Switch=1，以及player.py中的dt=0.1
...
shoplistfile = 'D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_new1'  #保存文件数据所在文件的文件名
shoplistfile_test = 'D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_d_test2'  #保存文件数据所在文件的文件名
shoplistfile_test1 = 'D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MASAC_compare'  #保存文件数据所在文件的文件名
...
class Memory():
    ...
    def store_transition(self,s,a,r,s_):
        # s和a应为所有智能体拼接后的数组
        # tran = np.hstack((s.flatten(), a.flatten(), r.flatten(), s_.flatten()))
...
def run(env):
    if Switch==0:
            ...
            all_ep_r2 = [[] for i in range(TRAIN_NUM)]
            for k in range(TRAIN_NUM):
                ...
                for episode in range(EP_MAX):
                    ...
                    reward_totle,reward_totle0,reward_totle1,reward_totle2 = 0,0,0,0
                    for timestep in range(EP_LEN):
                        ...
                        #     if not win[i]:
                        #         np.append(oba,observation[i].flatten())
                        #         np.append(aca, action[i].flatten())
                        #         np.append(rea, reward[i].flatten())
                        #         np.append(oba_,observation_[i].flatten())
                        # M.store_transition(oba, aca, rea,oba_)
                        ...
                        if M.memory_counter > MemoryCapacity:
                            ...
                            #     # TODO 方法零修改
                            #     # if not done[i]:
                            #     new_action, log_prob_ = actors[i].evaluate(b_s_[:, state_number * i:(state_number+1) * i])
                            ...
                            #     current_q1, current_q2 = critics[i].get_v(b_s,
                            #                                               b_a[:, action_number * i:(action_number * i ])
                    ...
                    all_ep_r2[k].append(reward_totle2)
                    if episode % 20 == 0 and episode > 200:#保存神经网络参数
                        save_data = {'net': actors[0].action_net.state_dict(), 'opt': actors[0].optimizer.state_dict()}
                        torch.save(save_data, "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_L1.pth")
                        save_data = {'net': actors[1].action_net.state_dict(), 'opt': actors[1].optimizer.state_dict()}
                        torch.save(save_data, "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_F1.pth")
                        save_data = {'net': actors[2].action_net.state_dict(), 'opt': actors[2].optimizer.state_dict()}
                        torch.save(save_data,"D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_E1.pth")
            ...
            all_ep_E_mean = np.mean((np.array(all_ep_r2)), axis=0)
            all_ep_E_std = np.std((np.array(all_ep_r2)), axis=0)
            d = {"all_ep_r_mean": all_ep_r_mean, "all_ep_r_std": all_ep_r_std,
                 ...
                 "all_ep_E_mean": all_ep_E_mean, "all_ep_E_std": all_ep_E_std,}
            ...
            all_ep_E_max = all_ep_E_mean + all_ep_E_std * 0.95
            all_ep_E_min = all_ep_E_mean - all_ep_E_std * 0.95
            plt.margins(x=0)
            ...
            plt.fill_between(np.arange(len(all_ep_F_mean)), all_ep_F_max, all_ep_F_min, alpha=0.6,
                             facecolor='#e75840')
            plt.xlabel('Episode')
            plt.ylabel('Hero1 reward')
            plt.figure(4, figsize=(8, 4), dpi=150)
            plt.margins(x=0)

            plt.plot(np.arange(len(all_ep_E_mean)), all_ep_E_mean, label='MASAC', color='#e75840')
            plt.fill_between(np.arange(len(all_ep_E_mean)), all_ep_E_max, all_ep_E_min, alpha=0.6,
                             facecolor='#e75840')
            plt.xlabel('Episode')
            plt.ylabel('Hero2 reward')
            plt.legend()
            plt.show()
            env.close()
    else:
        print('SAC测试中...')
        aa = Actor()
        checkpoint_aa = torch.load(
            "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_L1.pth")
        aa.action_net.load_state_dict(checkpoint_aa['net'])
        bb = Actor()
        checkpoint_bb = torch.load(
            "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_F1.pth")
        bb.action_net.load_state_dict(checkpoint_bb['net'])
        cc = Actor()
        checkpoint_cc = torch.load(
            "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_SAC_actor_E1.pth")
        cc.action_net.load_state_dict(checkpoint_cc['net'])
        action = np.zeros((N_Agent + M_Enemy, action_number))
        win_times = 0
        average_FKR = 0
        average_timestep = 0
        ...
        for j in range(TEST_EPIOSDE):
            ...
            for timestep in range(EP_LEN):
                ...
                action[2] = cc.choose_action(state[2])
                # for i in range(N_Agent):
