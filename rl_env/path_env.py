...
class RlGame(gym.Env):
    ...
    def __init__(self, n,m,render=False):
        ...
        if self.Render:
            ...
            self.GRAPHICS = tools.load_graphics('D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning/assignment\source\image')

            self.SOUND = tools.load_sound('D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning/assignment\source\music')
    ...
    def set_hero_image(self):
        ...
        self.hero['hero'+str(0)]=player.Hero(image=self.hero_image, x=200,y=500)
        self.hero_group.add(self.hero['hero'+str(0)])
        self.hero['hero' + str(1)] = player.Hero(image=self.hero_image, x=300, y=500)
        self.hero_group.add(self.hero['hero' + str(1)])
        self.hero['hero' + str(2)] = player.Hero(image=self.hero_image, x=400, y=500)
        self.hero_group.add(self.hero['hero' + str(2)])
    ...
    def set_hero(self):
        ...
        #     self.hero_group.add(self.hero['hero'+str(i)])
        self.hero['hero' + str(0)] = player.Hero(x=200, y=500)
        self.hero_group.add(self.hero['hero' + str(0)])
        self.hero['hero' + str(1)] = player.Hero( x=300, y=500)
        self.hero_group.add(self.hero['hero' + str(1)])
        self.hero['hero' + str(2)] = player.Hero(x=400, y=500)
        self.hero_group.add(self.hero['hero' + str(2)])
    ...
    def set_goal_image(self):
        ...
        self.goal['goal' + str(0)] = player.Goal(image=self.goal_image,x=200,y=200)
        self.goal_group.add(self.goal['goal' + str(0)])
        self.goal['goal' + str(1)] = player.Goal(image=self.goal_image, x=350, y=200)
        self.goal_group.add(self.goal['goal' + str(1)])
        self.goal['goal' + str(2)] = player.Goal(image=self.goal_image, x=500, y=200)
        self.goal_group.add(self.goal['goal' + str(2)])
    ...
    def set_goal(self):
        ...
        #     self.goal_group.add(self.goal['goal'+str(i)])
        self.goal['goal' + str(0)] = player.Goal( x=200, y=200)
        self.goal_group.add(self.goal['goal' + str(0)])
        self.goal['goal' + str(1)] = player.Goal(x=350, y=200)
        self.goal_group.add(self.goal['goal' + str(1)])
        self.goal['goal' + str(2)] = player.Goal( x=500, y=200)
        self.goal_group.add(self.goal['goal' + str(2)])
    ...
    def reset(self):#reset的仅是环境状态，
        ...
        self.team_counter = 0
        self.done = False
        self.hero_state = np.zeros((self.hero_num+self.enemy_num,7+(self.hero_num-1)))
        self.hero_α = np.zeros((self.hero_num, 1))
        # self.goal_x,self.goal_y=random.randint(100, 500), random.randint(100, 200)
        # return np.array(
            # [[self.hero0.init_x / 1000, self.hero0.init_y / 1000, self.hero0.speed / 30, self.hero0.theta * 57.3 / 360
        ...
        return np.array([[self.hero0.init_x / 1000, self.hero0.init_y / 1000, self.hero0.speed / 30, self.hero0.theta * 57.3 / 360
                         , self.goal0.init_x / 1000, self.goal0.init_y / 1000, 0,0,0],
                         [self.hero1.init_x / 1000, self.hero1.init_y / 1000, self.hero1.speed / 30,self.hero1.theta * 57.3 / 360,
                          self.goal1.init_x / 1000, self.goal1.init_y / 1000, 0,0,0],
                         [self.hero2.init_x / 1000, self.hero2.init_y / 1000, self.hero2.speed / 30,self.hero2.theta * 57.3 / 360
                         , self.goal2.init_x / 1000, self.goal2.init_y / 1000, 0,0,0],
        # return np.array([[self.hero0.init_x/1000,self.hero0.init_y/1000,self.hero0.speed/30,self.hero0.theta*57.3/360
        #                     ,self.goal0.init_x/1000, self.goal0.init_y/1000,0],
    ...
    def step(self,action):
        ...
        # 各个UAV之间的距离
        dis_i_to_other=np.zeros((self.hero_num,self.hero_num))
        r=np.zeros((self.hero_num+self.enemy_num, 1))
        o_flag = 0
        o_flag1 = 0
        h_flag=np.zeros((self.hero_num,1))
        #空气阻力系数
        F_k=0.08
        ...
        # speed_r=0
        # （4）避撞奖励
        crash_r=np.zeros((self.hero_num,1))
        # XXX: (6)时间惩罚 -- 让UAV尽可能快地到达目的地
        time_penalty = -1.0
        # XXX: 障碍物位置更新(仅测试时开启)
        # if self.Render:
        #     self.obstacle0.update()
        self.obstacle0.update()
        # for i in range(self.hero_num):
        #     print(self.hero['hero'+str(i)].posx,self.hero['hero'+str(i)].posy)
        ...
        for i in range(self.hero_num+self.enemy_num):
            ...
            # 如果UAV已成功到达目标
            if self.hero['hero' + str(i)].win:
                self.hero_state[i]=[0,0,0,0,0,0,0,0,0]
            # 计算到达障碍物和目标的距离
            else:
                dis_1_obs[i] = math.hypot(self.hero['hero' + str(i)].posx - self.obstacle0.init_x,
                                          self.hero['hero' + str(i)].posy - self.obstacle0.init_y)
                # # 支持多个障碍物
                # dis_1_obs[i] = math.hypot(self.hero['hero' + str(i)].posx - self.obstacle['obstacle' + str(i)].init_x,
                #                           self.hero['hero' + str(i)].posy - self.obstacle['obstacle' + str(i)].init_y)
                dis_1_goal[i] = math.hypot(self.hero['hero' + str(i)].posx - self.goal['goal'+str(i)].init_x,
                                           self.hero['hero' + str(i)].posy - self.goal['goal'+str(i)].init_y)
                
                # 计算当前UAV i与各个UAV之间的距离
                for j in range(self.hero_num):
                    dis_i_to_other[i][j]=math.hypot(self.hero['hero'+str(i)].posx - self.hero['hero'+str(j)].posx,
                                             self.hero['hero'+str(i)].posy - self.hero['hero'+str(j)].posy)
                
                # XXX：将越界设为-2，避免UAV一直在边缘徘徊
                # 越界检查（1）
                ...
                #         # ---------训练注释------------
                #         # and dis_1_agent_0_to_2<50 and dis_1_agent_0_to_3<50and dis_1_agent_0_to_4<50
                #         ):
                #     follow_r0=0
                #     self.team_counter+=1
                ...
                if not self.hero['hero' + str(i)].dead:
                    for j in range(self.hero_num):
                        if i!=j and not self.hero['hero' + str(j)].win and dis_i_to_other[i][j]<5:
                            h_flag[j]=1
                            crash_r[i]=-300  #Masac奖励
                            # crash_r[i] = -250
                            self.hero['hero' + str(i)].die()
                            self.hero['hero' + str(i)].win = False
                            self.done = True
                            print('hero',j, 'crash_gg')
                        elif i!=j and not self.hero['hero' + str(j)].win and dis_i_to_other[i][j]<10:
                            h_flag[j] = 1
                            crash_r[i]=-2 if crash_r[i]==0 else crash_r[i]
                        # elif i!=j and self.hero['hero' + str(j)].win:

                    # goal_r[i] = 10.0 / (dis_1_goal[i] + 1e-5)
                ...
                if dis_1_goal[i] < 40 and not self.hero['hero' + str(i)].dead:
                    ...
                    self.done = True
                    for j in range(self.hero_num):
                        if not self.hero['hero'+str(j)].win:
                            self.done=False
                    # if not (self.hero['hero'+str(0)].win and self.hero['hero'+str(1)].win):
                    #     self.done=False
                    print('hero'+str(i)+'aa')
                    # if self.done:
                    #     goal_r[i]+=1000
                ...
                # （2.2）UAV与障碍物距离突破安全阈值（半径的2倍）
                elif dis_1_obs[i] < 40 and not self.hero['hero' + str(i)].dead:
                    o_flag = 1
                    # print(-100000*math.pow(1/dis_1_obs[i],2))
                    obstacle_r[i] = -10  #-100000*math.pow(1/dis_1_obs[i],2)
                # UAV与终点距离的动态奖励
                elif not self.hero['hero' + str(i)].dead:
                    # print(math.exp(100/dis_1_goal[i])/10)
                    goal_r[i] =-0.005 * dis_1_goal[i]# math.exp(100/dis_1_goal[i])/10
                # （5）能耗奖励
                # 添加能耗奖励计算（动作的L2范数平方）
                energy_cost = -0.01 * (action[i][0] ** 2 * action[i][1] ** 2)  # 能耗惩罚系数设为0.01
                # energy_cost=0
                # UAV i 的奖励计算
                r[i] = edge_r[i] + obstacle_r[i] + goal_r[i] + crash_r[i] + energy_cost + time_penalty
                # r[i] = edge_r[i] + obstacle_r[i] + goal_r[i]+speed_r+follow_r0
                # r[i] = edge_r[i] + obstacle_r[i] + goal_r[i] + crash_r[i]
                # 更新状态向量
                self.hero_state[i][:7] = [
                    self.hero['hero' + str(i)].posx / 1000,
                    self.hero['hero' + str(i)].posy / 1000,
                    self.hero['hero' + str(i)].speed / 30,
                    self.hero['hero' + str(i)].theta * 57.3 / 360,
                    self.goal['goal'+str(i)].init_x / 1000,
                    self.goal['goal'+str(i)].init_y / 1000,
                    o_flag
                ]
                # 将队友信息存入状态
                k=0
                for j in range(self.hero_num):
                    if i!=j:
                        # self.hero_state[i][7+3*k]=self.hero['hero' + str(j)].posx / 1000
                        # self.hero_state[i][7+3*k+1]=self.hero['hero' + str(j)].posy / 1000
                        self.hero_state[i][7+k]=h_flag[j]
                        k+=1
                # for j in range(self.hero_num):
                #     self.hero_state[7+j]=h_flag[j]
        ...
        # ---------训练注释------------删除dis_1_agent_0_to_1
        # return hero_state, r, done, self.hero['hero0'].win, self.team_counter
        # return hero_state, r, done, self.hero['hero0'].win and self.hero['hero1'].win, self.team_counter
        win=np.zeros((self.hero_num,1))
        for i in range(self.hero_num):
            win[i]=self.hero['hero'+str(i)].win
        return hero_state, r, done, win, self.team_counter
        # return hero_state, r, done, self.hero['hero0'].win and self.hero['hero1'].win and self.hero['hero2'].win, self.team_counter
    ...
    def draw(self,surface):
        ...
        for i in range(self.hero_num):
            pygame.draw.circle(surface, C.RED, (self.goal['goal'+str(i)].init_x, self.goal['goal'+str(i)].init_y), 1)
            pygame.draw.circle(surface, C.RED, (self.goal['goal'+str(i)].init_x, self.goal['goal'+str(i)].init_y), 40, 1)
