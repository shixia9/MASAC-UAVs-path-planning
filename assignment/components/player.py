...
class Hero(GameSprite):
    def __init__(self,image=None,x=0,y=0):
        ...
        if x==0 and y==0:
            ...
            # self.rect.center = random.randint(100, 400), random.randint(100, 550)
        else:
...
class Obstacle(GameSprite):
    '''障碍'''
    def __init__(self,image=None):
        super(Obstacle, self).__init__(image_name=image,size=(30,30))
        #自方的初始位置
        self.rect.center=350, random.randint(350,400)
        self.init_x,self.init_y=self.rect.center
        self.posx, self.posy = self.init_x, self.init_y

        # 添加随机初速度和方向
        self.speed = random.uniform(5, 15)  # 像素/秒
        self.theta = random.uniform(0, 2 * math.pi)  # 移动方向（弧度）
        self.target_theta = self.theta  # 初始时目标角度与当前一致
        self.turn_speed = math.radians(30)  # 每秒最多转30°
        # 添加变向控制
        self.last_theta_change_time = time.time()
        self.theta_change_interval = random.uniform(1.0, 2.0)  # 每隔 1~2 秒调整一次方向
    
    def update(self):
        # 随机变向
        now = time.time()
        if now - self.last_theta_change_time > self.theta_change_interval:
            self.target_theta = random.uniform(0, 2 * math.pi)
            self.last_theta_change_time = now
            self.theta_change_interval = random.uniform(1.0, 2.0)  # 设置下一次变向间隔
        # 平滑调整 theta -> target_theta
        angle_diff = self.target_theta - self.theta
        # 把角度差规整到 [-π, π]，避免跨0弧度导致方向跳变
        ...
        if abs(angle_diff) <= max_turn:
            ...
            self.theta += max_turn * np.sign(angle_diff)

        # 每个step更新一次
        self.posx += self.speed * math.cos(self.theta) * dt
        self.posy -= self.speed * math.sin(self.theta) * dt
        # 边界检测、反弹
        if self.posx <= C.ENEMY_AREA_X or self.posx >= C.ENEMY_AREA_X + C.ENEMY_AREA_WITH:
            self.theta = math.pi - self.theta
        if self.posy <= C.ENEMY_AREA_Y or self.posy >= C.ENEMY_AREA_Y + C.ENEMY_AREA_HEIGHT:
...
class Goal(GameSprite):
    ...
    def __init__(self,image=None,x=0,y=0):
        ...
        if x==0 and y==0:
            ...
            self.rect.center = random.randint(300, 350), random.randint(150, 200)
            self.init_x, self.init_y = self.rect.center
        else:
            self.rect.center = x, y
            self.init_x, self.init_y = self.rect.center
