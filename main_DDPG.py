...
shoplistfile = 'D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MADDPG'  #保存文件数据所在文件的文件名
shoplistfile_test = 'D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\MADDPG_compare'  #保存文件数据所在文件的文件名
...
def run(env):
    if Switch==0:
        ...
        for k in range(TRAIN_NUM):
            ...
            for episode in range(EPIOSDE_ALL):
                ...
                if episode % 50 == 0 and episode > 200:#保存神经网络参数
                    ...
                    torch.save(save_data, "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_new.pth")
                    save_data = {'net': actors[1].actor_estimate_eval.state_dict(), 'opt': actors[1].optimizer.state_dict()}
                    torch.save(save_data, "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_1_new.pth")
                    save_data = {'net': actors[2].actor_estimate_eval.state_dict(), 'opt': actors[2].optimizer.state_dict()}
                    torch.save(save_data, "D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_2_new.pth")
        ...
        plt.plot(np.arange(len(all_ep_E_mean)), all_ep_E_mean, label='MADDPG', color='#e75840')
        ...
        # cc.actor_estimate_eval.load_state_dict(checkpoint_cc['net'])
        aa = Actor()
        checkpoint_aa = torch.load("D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_new.pth")
        aa.actor_estimate_eval.load_state_dict(checkpoint_aa['net'])
        bb = Actor()
        checkpoint_bb = torch.load("D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_1_new.pth")
        bb.actor_estimate_eval.load_state_dict(checkpoint_bb['net'])
        cc = Actor()
        checkpoint_cc = torch.load("D:/drlTest/UAV-path-planning-main\Multi-UAVs path planning\path planning\Path_DDPG_actor_2_new.pth")
        cc.actor_estimate_eval.load_state_dict(checkpoint_cc['net'])
        action = np.zeros((N_Agent + M_Enemy, action_number))
        win_times = 0
        average_FKR = 0
